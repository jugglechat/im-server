...
func MobileNaviGet(ctx *gin.Context) {
			...
			if err == nil {
				tools.SuccessHttpResp(ctx, models.DefaultResp{
					...
					Data: models.NaviResp{
						AppKey:  appKey,
						UserId:  token.UserId,
						Servers: []string{fmt.Sprintf("%s:%d", configures.Config.ConnectManager.TcpHost, configures.Config.ConnectManager.TcpPort)},
					},
				})
				return
			} else {
...
func MobileNavi(ctx *gin.Context) {
